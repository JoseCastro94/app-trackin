...
const ReporteDevolucionPage = () => {
    ...
    const handleChangeDate = (isIni, date) => {
        ...
        isIni ? setFechaIni(fecha) : setFechaFin(fecha)
    ...
    const loadPage = React.useCallback(async () => {
        ...
        const estados = await listarParametros(process.env.REACT_APP_ID_GRUPO_PARAMETRO_ESTADOS_DESPACHO_DEVOLUCION, token, tokenCompany);
    ...
    const search = async (pageInit = page, limit = rowsPerPage) => {
        ...
        const response = await reporteDevolucion(filtros, token, tokenCompany)
        setRows(response)
    }
    ...
    const handleExportExcel = async () => {
        ...
        const data = await exportExcelDevoluciones(filtros, token, tokenCompany)
        await downloadExcel(data, 'reporte devoluciones')
    }
