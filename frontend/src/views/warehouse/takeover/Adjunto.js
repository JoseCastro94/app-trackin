...
const AdjuntoTakeover = React.forwardRef(({
    ...
    const handleFileUpload = (e) => {
        ...
        fetch(process.env.REACT_APP_API + 'business/api/relevo_adjunto/file/' + edit, {
            method: 'POST',
            body: data,
            headers: {
                'token': token,
    ...
    const handleRemoveFile = (IdAdjunto) => {
        return () => {
            fetch(`${process.env.REACT_APP_API}business/api/relevo_adjunto/delFile/${IdAdjunto}`, {
                method: 'GET',
                headers: {
                    'Content-Type': 'application/json',
                    'token': token,
...
export default AdjuntoTakeover