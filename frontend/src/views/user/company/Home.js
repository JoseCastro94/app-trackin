...
const UserCompanyHome = () => {
    ...
    const procesar = (ids, id) => {
        fetch(process.env.REACT_APP_API + 'business/api/usuario_empresa/updUsuarioEmpresa', {
            method: 'POST',
            body: JSON.stringify({
                IdUsuario: id,
                listEmpresa: ids
    ...
    const [rowsEmpresa, setRowsEmpresa] = React.useState([])

    const columnsNegocio = [
        { field: 'Ruc', headerName: 'Ruc', width: 250 },
        { field: 'RazonSocial', headerName: 'RazonSocial', width: 350 },
        { field: 'id', hide: true },
    ]

    React.useEffect(() => {
        if (idUsuario) {
            fetch(process.env.REACT_APP_API + 'business/api/empresaparametro/UsuarioEmpresa', {
                method: 'POST',
                body: JSON.stringify({
                    IdUsuario: idUsuario
                }),
                ...
                .catch(error => console.error('Error:', error))
                .then(response => {
                    setRowsEmpresa(response.empresas)
...
export default UserCompanyHome