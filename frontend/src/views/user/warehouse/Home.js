...
const UserBusinessHome = () => {
    ...
    const procesar = (ids, id) => {
        fetch(process.env.REACT_APP_API + 'business/api/usuarioalmacen/updUsuarioAlmacen', {
            method: 'POST',
            body: JSON.stringify({
                IdUsuario: id,
                listAlmacen: ids
    ...
    React.useEffect(() => {
        if (idUsuario) {
            fetch(process.env.REACT_APP_API + 'business/api/almacen/UsuarioAlmacen', {
                method: 'POST',
                body: JSON.stringify({
                    IdUsuario: idUsuario
                }),
                ...
                .catch(error => console.error('Error:', error))
                .then(response => {
                    setRowsNegocio(response.almacenes)
    ...
    return (
        <Container fixed>
            <Grid container spacing={gridSpacing}>
                <Grid item xs={12}>
                    <Typography variant='h3' sx={{ color: theme.palette.primary.main }}>Usuario - Almacen</Typography>
                </Grid>
                <Grid item xs={12}>
                    <DataGridApp
                        rows={rows}
