import { TransferenciaAdjunto } from '../models/TransferenciaAdjunto.js'
...
export const upFile = async (req, res) => {
    ...
    try {
        ...
        uploadParams.Key = `transferencias/${year}/${month}/${uuidv4()}.${ext}`
        ...
        s3.upload(uploadParams, async (err, data) => {
            if (err) {
                ...
                let newTransferenciaAdjunto = await TransferenciaAdjunto.create(
                    {
                        Key: data.Key,
                        ...
                        IdDetalleTranslado: IdDetalleTranslado
                    }
                )
                res.json(newTransferenciaAdjunto)
            }
        })
    } catch (e) {
...
export const delFile = async (req, res) => {
    try {
        ...
        await TransferenciaAdjunto.destroy({
            where: {
                IdAdjunto
            }
        })
...
export const getFile = async (req, res) => {
    try {
        ...
        const attached = await TransferenciaAdjunto.findOne({
            where: {
                IdAdjunto
            }
        })
...
export const getAttacheds = async (req, res) => {
    try {
        ...
        const adjuntos = await TransferenciaAdjunto.findAll({
            attributes: [
                "IdAdjunto",
                "Key",
                "Nombre",
